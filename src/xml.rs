...
impl Tree {
    ...
    fn fmt_rec(&self, f: &mut fmt::Formatter<'_>, depth : usize) ->
        Result<(), fmt::Error>
    {
        for _ in 0..depth {
            write!(f, "\t")?;
...
impl TreeStep {
    fn return_next<T>(&self, next : (TreeStep, Option<T>)) ->
        (TreeStep, Option<T>)
    {
        let (step, val) = next;
        (*self.min(&step), val)
...
pub struct Return<S, T> {
    f: Box<dyn Fn(&mut S, bool) -> Option<Option<T>>>
}

impl<T> Return<WalkState, T>
where
    T : 'static,
{
...
impl Return<WalkState, ()> {
    fn nil() -> Self {
        Self::from_lambda( |state, revisit| { Some(None) })
    }
}

impl Return<WalkState, String> {
...
impl Return<WalkState, Vec<(String, String)>> {
...
impl<T> Matcher<WalkState, T>
where
    T : 'static,
{

    pub fn new(r : Return<WalkState, T>) -> Self {
        Self {
            f: Box::new(move |state, revisit| {
                (r.f)(state, revisit)
            })
        }
    }

    fn from_parts<F>(f : F) -> Self
    where
        F : Fn(&mut WalkState, bool) -> Option<Option<T>> + 'static,
    {
        Self {
    ...
    pub fn tag_eq(self, s : String) -> Self {
        Self::from_parts(move |state, revisit| {
    ...
    pub fn and<U : 'static>(self, next : Matcher<WalkState, U>) ->
        Matcher<WalkState, U>
    {
        Matcher::from_parts(move |state, revisit| {
    ...
    pub fn and_then<U : 'static, F>(self, next : F) -> Matcher<WalkState, U>
    where
        F : Fn(T) -> Matcher<WalkState, U> + 'static
    {
        Matcher::from_parts(move |state, revisit| {
    ...
    T : 'static,
{
    ...
    where
        F : Fn() -> Matcher<WalkState, T> + 'static
    {
        Self::from_parts(move |state, revisit| {
...
where
    T : 'static,
{
    pub fn matches<R : io::Read + 'static>(self, r : EventReader<R>) ->
        MatchIter<T>
    {
        MatchIter {
            m : self,
...
pub struct WalkState {
    //r : Rc<dyn Iterator<Item = XmlResult<XmlEvent>>> , // Events<R> or Tee<XmlEvent>

    // this doesn't have to be Rc it's a hack for garbage collection in the
    // BacktrackerBuf
    r : Rc<RefCell<
        Backtracker<
            Box<dyn Iterator<Item = XmlResult<XmlEvent>>>,
            XmlResult<XmlEvent>
        >
    >>,
...
impl WalkState {
        ...
        loop {
            ...
            match evt {
                ...
                XmlEvent::EndElement{name} => {
                    ...
                    if self.depth > 0 {
                        ...
                        self.current = self.current.as_ref().and_then(|rc| {
                            rc.borrow().parent.as_ref().and_then(|weak| {
                                Weak::upgrade(weak)
                            })
                        });
                    } else {
                ...
                XmlEvent::Characters(s) => {
                    let tree = Tree::Text(
                        self.current.as_ref().map(|rc| Rc::downgrade(rc)),
                        s);
                    if let Some(current) = self.current.as_mut() {
                        current.borrow_mut()
                            .add_child(tree);
    ...
    fn should_revisit(&mut self) -> bool {
        let ret = self.revisits.last().map_or(false, |depth| *depth == self.depth);
    ...
    fn did_revisit(&mut self) {
        let hasnt_revisited =
            self.revisits.last().map_or(false, |depth| *depth == self.depth);
        if hasnt_revisited {
            self.revisits.pop();
        }
    }
